...
function getTreasureSprite(size) {
  ...
  function drawCompactSelectorField(targetCtx, x, y, width, height, label, value, onClick, register, clipRect) {
    ...
    if (onClick && (register !== false)) {
      ...
      if (clipRect) {
        registerClippedRegion(target, x, y + 22, width, height, clipRect.x, clipRect.y, clipRect.width, clipRect.height, onClick);
      } else {
        registerRegion(target, x, y + 22, width, height, onClick);
      }
    }
  }
