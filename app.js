...
function getTreasureSprite(size) {
  ...
  function drawCompactSelectorField(targetCtx, x, y, width, height, label, value, onClick) {
    ...
    if (onClick) {
      ...
      registerRegion(targetCtx === overlayCtx ? overlayRegions : mainRegions, x, y + 22, width, height, onClick);
    }
  }
  ...
  function drawEditorChoiceRow(targetCtx, x, y, width, rowKey, title, buttons) {
    ...
    if (canLeft) registerRegion(overlayRegions, x, rowY, arrowW, rowH, function () { scrollEditorChoiceRow(rowKey, -220, maxScroll); });
    if (canRight) registerRegion(overlayRegions, x + width - arrowW, rowY, arrowW, rowH, function () { scrollEditorChoiceRow(rowKey, 220, maxScroll); });
    targetCtx.save();
    targetCtx.beginPath();
    ...
    buttons.forEach(function (button) {
      const visible = cursorX + button.width > viewportX && cursorX < viewportX + viewportW;
      if (visible) {
        ...
        registerRegion(overlayRegions, Math.max(cursorX, viewportX), rowY, Math.min(button.width, viewportX + viewportW - cursorX), rowH, button.onClick);
      }
  ...
  function renderEditorModal(targetCtx, view) {
    ...
    selectorRows[0].forEach(function (entry, index) {
      drawCompactSelectorField(targetCtx, selectorRowOneX + index * (selectorW + selectorGapX), selectorRowOneY, selectorW, selectorFieldH, entry.label, entry.value, entry.onClick);
    });
    selectorRows[1].forEach(function (entry, index) {
      drawCompactSelectorField(targetCtx, selectorRowTwoX + index * (selectorW + selectorGapX), selectorRowTwoY, selectorW, selectorFieldH, entry.label, entry.value, entry.onClick);
    });
